...
void CDestroyBigMatrix2(SEXP bigMatrixAddr)
{
...
SEXP GrabVideo( SEXP T ) {
  ...
  for(i=0; i < INTEGER_VALUE(T); ++i)
  {
    ...
    if (i==0) {
      ...
      bm->create(frameSize.width*frameSize.height,
                 INTEGER_VALUE(T), 1, false );
    }
...
//        imshow("edges", edges);
  ...
  R_RegisterCFinalizerEx(ret, (R_CFinalizer_t) CDestroyBigMatrix2,
    (Rboolean) TRUE);

...
worker.km <- function(i, desc) {
  ...
  clust <- big.matrix(nrow(x), 1, type='integer', init=0)
                                                  # BIG * 1 (holds integers)
  ...
  # Now ensure that our centers are unique, and if not, resample:
  for (ii in 1:(10+2^k)) {
    if (any(duplicated(cent[,]))) {
      cent[,] <- x[sample(1:n, k),]
    } else break;
  }
  ...
  if (!any(duplicated(cent[,]))) {
    iters = .Call("Ckmeans2main", as.integer(mattype),                   
                  ...
                  as.double(n), as.integer(k), as.double(m),
                  as.integer(max.iters))
    ans <- list(cluster=clust[,], centers=cent[,], withinss=ss[,],
                size=clustsizes[,], iters=iters)
    class(ans) <- "kmeans"
  } else {
    ans <- "Error with non-unique starting points."
  }
  return(ans)
...
{
  warning("is.shared is being called.  This needs to be updated!")
  return(TRUE)
}
kmeans.big.matrix <- function(x, centers, iter.max = 10, nstart = 1,
  ...
  if (is.big.matrix(x)) {
    ...
    # If the matrix is a shared object, then get the description, 
    # otherwise pass the matrix pointer.
    if (is.shared(x)) {
      desc[[1]] <- describe(x)
    } else {
      desc[[1]] <- x
  ...
  if (is.shared(x) && parallel=="nws" && nstart>1 && !is.null(nwssleigh)) {
    ...
    ans <- eachElem(nwssleigh, worker.km, elementArgs=rep(1,nstart),
                    fixedArgs=list(desc))
    best <- min(which(unlist(lapply(ans,class))=="kmeans"))
    if (best==Inf) stop("Uniqueness problem in getting centers.")
    ii <- best
    if (length(ans)>1) {
      for (i in ii:length(ans)) {
        if (class(ans)=="kmeans") {
          if (sum(ans[[i]]$withinss) < sum(ans[[best]]$withinss)) best <- i
        } else {
          warning("Problem generating unique starting centers.\n")
        }
      }
    }
    ans <- ans[[best]]
    ...
    if (class(ans)!="kmeans") ans <- list(withinss=Inf)
    if (nstart>1) {
      for (i in 2:nstart) {
        temp <- worker.km(i, desc)
        if (class(temp)=="kmeans") {
          if (sum(temp$withinss) < sum(ans$withinss)) ans <- temp
        } else {
          warning("Problem generating unique starting centers.\n")
        }
      }
      if (ans$withinss[1]==Inf) stop("Uniqueness problem in getting centers.")
    }
